package com.worldline.easycukes.arquillian.example.controller;
	...
	public String eatCukes() {
		if (belly.isHungry()) {
			facesContext.addMessage(null, new FacesMessage(SEVERITY_ERROR,
					MSG_ERROR_HUNGRY, MSG_ERROR_HUNGRY));
		} else {
			String message = MessageFormat.format(MSG_SUCCESS_NOT_HUNGRY,
					belly.getCukes());
			facesContext.addMessage(null, new FacesMessage(SEVERITY_INFO,
					message, message));
		}
		return "belly";
	}
