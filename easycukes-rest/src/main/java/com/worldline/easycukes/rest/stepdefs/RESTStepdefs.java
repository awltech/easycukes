...
public class RESTStepdefs {
	...
	public void checkAResponseProperty(final String property,
									   final String result) throws java.text.ParseException {
		...
		try {
			...
			Assert.assertTrue(expectedResult.equals(RestService.getInstance().getResponseProperty(
					DataInjector.injectData(property))));
		} catch (java.text.ParseException | ParseException parseException) {
	...
	public void checkIfResponsePropertyIsEmpty(final String property) throws ParseException {
		log.debug("Asserts that the response property " + property + " has a value ");
		try {
	...
	public void validateAResponsePropertyEqualsTo(final String property, final String result) throws ParseException {
		log.info("Asserts that the response property equals " + result);
		try {
			Assert.assertTrue(DataInjector.injectData(result).equals(
					RestService.getInstance().getResponseProperty(DataInjector.injectData(property))));
		} catch (ParseException parseException) {
			log.error(parseException.getMessage());
		}
	}
	...
	public void validateAResponsePropertyDoesntContain(final String property, final String result)
			throws ParseException {
		log.info("Asserts that the response property not contains " + result);
		try {
			Assert.assertFalse(RestService.getInstance().getResponseProperty(DataInjector.injectData(property))
	...
	public void validateAResponsePropertyContains(final String property, final String result)
	...
	public void setAParameterToAResponseProperty(final String param, final String property) throws ParseException {
	...
	public void setAParameterToAResponseContent(final String param) {
		...
		CukesHelper.setParameter(param, RestService.getInstance().getResponseContent());
	}


	@Given("^I set the parameter \"(.*?)\" to (today|yesterday|tomorrow)$")
	public void setAParameterToAKeyword(final String param,
										final String value) throws java.text.ParseException {
		CukesHelper.setParameter(param, DateHelper.getDateValue(value));
	}


	@Given("^I set the date parameter \"(.*?)\" to (\"?.*\"?)$")
	public void setAParameterToADate(final String param, final String value) throws java.text.ParseException {
		CukesHelper.setParameter(param, DateHelper.parseDateToJson(value));
	}


	@Given("^I set the parameter \"(.*?)\" to \"(.*?)\"$")
	public void setAParameterToADynamicValue(final String param, final String value) throws java.text.ParseException {
	...
	public void setAParameterToARandomPropertyFromResponseArray(
																final String param, final String property)
			throws ParseException {
		CukesHelper.setParameter(param, RestService.getInstance()
	...
	public void setAParameterToAFirstPropertyFromResponseArray(final String param, final String property) throws ParseException {
		CukesHelper.setParameter(param, RestService.getInstance()
	...
	public void setAParameterToARandomItemFromResponseArray(String param) throws ParseException {
		CukesHelper.setParameter(param, RestService.getInstance()
	...
	public void setRetryInterval(int timeToWait) {
	...
	public void sendGetRequestAndRetryTillResponsePropertyIs(String path,
															 String property, String result) throws Exception{
		log.info("Sending GET request to " + path + " and retry until response property " + property + " is " + result);
		try {
			String expectedResult = result;
			if (CukesHelper.isExpression(result))
			...
			log.error(parException.getMessage());
		}
	}
	...
	public void sendPostRequestWithParameters(String url, String params) throws Exception {
	...
	public void setAParameterToResponseHeader(String param, String property) {
		CukesHelper.setParameter(param, RestService.getInstance().getPropertyFromResponseHeader(property));
	}


	@Then("^I set the request header \"(.*?)\" to \"(.*?)\"$")
	public void setTheRequestHeader(String header, String value) {
		RestService.getInstance().addRequestHeader(header, DataInjector.injectData(value));
	}
	...
	public void checkIfParameterEquals(String parameter, String value) {
		Assert.assertTrue(value.equalsIgnoreCase(ExecutionContext.get(parameter)));
	}
